...
zcta_joined <- mn_zctas %>%
  full_join(asthmaMN, by = c("GEOID20" = "_ZIP")) %>%
  drop_na() %>%
...
pattern_dep <- htmltools::htmlDependency(
  name = "leaflet.pattern",
  version = "0.2.0",
  src = c(href = "https://unpkg.com/leaflet.pattern@0.2.0/dist"),
...
plot <- leaflet(zcta_joined) %>%
  ...
  addPolygons(
    ...
    function(el, x) {
      ...
      // --- Define two patterns ---
      var patTrue  = new L.StripePattern({weight:6, spaceWeight:6, color:'#1f77b4', angle:45});
      var patFalse = new L.StripePattern({weight:6, spaceWeight:6, color:'#ff7f0e', angle:-45});
      patTrue.addTo(map);
      patFalse.addTo(map);

      // --- Apply patterns based on TRUE/FALSE ---
      map.eachLayer(function(layer){
        if(layer.feature && layer.feature.properties){
          var val = layer.feature.properties.ej_status;
          if(val === true){
            layer.setStyle({fillPattern: patTrue});
          } else {
            layer.setStyle({fillPattern: patFalse});
          }
        }
      });
    }
...
plot <- htmltools::attachDependencies(plot, pattern_dep)
plot
