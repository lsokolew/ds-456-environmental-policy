...
mn_counties_aqi %>% 
  filter(pollutant == "PM2.5") %>% 
  ggplot() +
    geom_line(aes(x = year, y = average_concentration, group = fossil_fuel, color = fossil_fuel), show.legend = FALSE) +
  labs(title = "Average PM2.5 Concentration By County") +
  ylim(0, 12) +
  theme_classic()
```
...
What's the modeling taking into account? can it/does it take powerplants into acount
color plot by whether there'e a pp or not
