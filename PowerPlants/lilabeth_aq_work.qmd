...
AirData_sf_proj <- st_transform(AirData_sf, 4326)

mn_pp_sf <- st_as_sf(mn_powerplants, coords = c("longitude", "latitude"), crs = 4326) %>% filter(fossil_fuel == "Fossil Fuel")
mn_pp_proj <- st_transform(mn_pp_sf, 4326)

# Buffer distance: 1 mile = 1609.34 meters
buffer_distance <- 1609.34*3

air_buffers <- st_buffer(AirData_sf_proj, dist = buffer_distance)



within_3miles_of_monitor <- st_within(mn_pp_proj, air_buffers)
mn_pp_sf$near_monitor <- lengths(within_3miles_of_monitor) > 0
# 46 plants are within 3 miles of a monitor

ggplot() +
  geom_sf(data=air_buffers) +
  geom_sf(data=mn_pp_sf, aes(color = near_monitor), size = .5)




# get count of pps within range of an air monitor
pp_within_each_monitor <- st_intersects(air_buffers, mn_pp_proj)

# Add the count as a column
air_buffers$nearby_pp_count <- lengths(pp_within_each_monitor)
...
leaflet() %>%
      ...
      addCircleMarkers(data = mn_powerplants, 
                       ...
                       radius = ~rescale(total_mw, to = c(1, 12)),
                       #stroke = FALSE
                       ) #%>%
      # addLegendFactor(
