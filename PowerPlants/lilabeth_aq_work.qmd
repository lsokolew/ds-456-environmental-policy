...
ff_status <- mn_powerplants %>%
  group_by(county) %>%
  summarize(has_fossil = any(fossil_fuel == "Fossil Fuel")) %>%
  mutate(plant_group = ifelse(has_fossil, "Fossil Fuel County", "Renewable/None"))


mn_counties_aqi %>%
  filter(pollutant == "PM2.5") %>%
  left_join(ff_status, by = c("name" = "county")) %>%
  mutate(plant_group = ifelse(is.na(plant_group), "Renewable/None", plant_group)) %>%
  ggplot(aes(x = year, y = average_concentration, group = name, color = plant_group)) +
    ...
    ylim(0, 12) +
    scale_color_manual(values = c("Fossil Fuel" = "#d95f02",
                                  "Renewable Only/None" = "#1b9e77")) +
    theme_classic()
...
mn_counties_aqi %>%
  ...
  ggplot(aes(x = year, y = average_concentration, group = name, color = plant_group)) +
    ...
    ylim(0, 12) +
    scale_color_manual(values = c("Fossil Fuel County" = "#d95f02",
                                  "Renewable/None" = "lightgreen")) +
    theme_classic()
```
...
look at documentation of MCAQ - is pps already accounted for? yes - the modeled AQ data uses powerplants' emissions in its modeling process
look at outliers 
...
- figure out what's going on with dates for plant 1912
...
- insert units into graph axis titles
