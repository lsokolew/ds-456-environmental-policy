...
#school_in_EJ <- st_within(school_proj,ej_tracts_proj)
#schools_sf$school_in_EJ <- lengths(school_in_EJ) > 0


#EJ_school_proj <- st_transform(EJ_school, 26915)
#EJ_school_buffer <- st_buffer(EJ_school_proj, dist = (1609.34))
#within_mile_EJ_school <- st_within(pp_points_proj,EJ_school_buffer)
#EJ_school$within_mile_EJ_school <- lengths(within_mile_EJ_school) > 0

points_sf %>%
  filter(fossil_fuel)

points_sf %>%
  ggplot(aes(x = within_mile_school)) +
  geom_bar()
...
within_mile <- st_within(pp_points_proj, tracts_buffer)
points_sf$within_mile <- lengths(within_mile) > 0
points_sf$within_mile
