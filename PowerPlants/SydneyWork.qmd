...
buffers <- st_buffer(points_sf, dist = 1609) # 1609 m = 1 mile

# Intersect with population data
affected <- st_intersection(buffers, ej_sf)

# Calculate affected population per buffer
...
leaflet() %>%
  addTiles() %>%
  addPolygons(data = buffers, fillColor = "red", opacity = 0.3) %>%
  addCircleMarkers(data = plants_sf, radius = 5, color = "black")
```
